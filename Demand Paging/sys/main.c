...
void proc1_test4(int* ret) {
	...
	//shutdown();
	//kprintf("\n proc1 test is leaving before sleep\n");
	sleep(6);
	kprintf("\n proc1 test is leaving after sleep\n");
	/*Shoud see what proc 2 updated*/
	for (i = 0; i < 26; i++) {
		/*expected output is abcde.....*/
		if (*(addr + i * NBPG) != 'a'+i){
			kprintf("\nproc1 condition failed\n");
			*ret = TFAILED;
			kprintf("\n proc1 test is leaving before break;\n");
			break;
		}
		//kprintf("0x%08x: %c\n", addr + i * NBPG, *(addr + i * NBPG));
	}
	//shutdown();
	kprintf("\n proc1 test is leaving \n");
	xmunmap(MYVPNO1);
	release_bs(MYBS1);
	//kprintf("\n proc1 test is leaving \n");
	return;
