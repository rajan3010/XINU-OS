...
/*Queue initialization for page replacement policy*/
struct 	policy_q * schead;	/* head of sc queue	*/
struct  policy_q * aginghead; /*head of aging queue */
struct  policy_q * sc_curr; /*current pointer for the sc queue*/
...
*/
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	// Initializing paging details starts from here

	init_bsm(); /* Initialize backing store and frames */
	init_frm();
	...
	/* First 4 frames are global page tables*/
	for(ind=0; ind<4; ind++){
